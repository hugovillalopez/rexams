...
function assignSizeVariables(variables, sizes2, name) {
  keys(sizes2).forEach(
    (size3) => Object.assign(variables, { [`--mantine-${name}-${size3}`]: sizes2[size3] })
  );
}
      ...
      ScrollAreaScrollbarX,
      {
        ...
        onThumbPositionChange: () => {
          if (context.viewport && thumbRef.current) {
            ...
            const offset3 = getThumbOffsetFromScroll(scrollPos, sizes2, dir);
          ...
          Box,
          {
            ...
            children: [
              ...
              (0, import_jsx_runtime140.jsxs)("div", { ...getStyles2("labelWrapper"), "data-disabled": disabled || void 0, children: [
                ...
                error2 && typeof error2 !== "boolean" && (0, import_jsx_runtime140.jsx)(Input.Error, { size: size3, __inheritStyles: false, ...getStyles2("error"), children: error2 })
              ] })
            ]
          }
        )
...
var Breadcrumbs = factory((_props, ref) => {
  ...
  const items = import_react242.Children.toArray(children).reduce((acc, child, index5, array) => {
    var _a;
    const item = isElement2(child) ? (0, import_react242.cloneElement)(child, {
...
var PillsInput = factory((_props, ref) => {
  ...
  return (0, import_jsx_runtime238.jsx)(PillsInputProvider, { value: { fieldRef, size: size3, disabled, hasError: !!error2, variant }, children: (0, import_jsx_runtime238.jsx)(
    InputBase,
    {
      size: size3,
      error: error2,
        ...
        children: [
          (0, import_jsx_runtime240.jsx)(Combobox.DropdownTarget, { children: (0, import_jsx_runtime240.jsx)(
            PillsInput,
            {
              ...
              disabled,
              radius,
              rightSection: rightSection || clearButton || (0, import_jsx_runtime240.jsx)(Combobox.Chevron, { size: size3, error: error2, unstyled }),
...
var NativeSelect = factory((props, ref) => {
  ...
  return (0, import_jsx_runtime242.jsx)(
    InputBase,
    {
      ...
      error: error2,
      unstyled,
      rightSection: rightSection || (0, import_jsx_runtime242.jsx)(ComboboxChevron, { size: size3, error: error2, unstyled }),
...
function isInRange(value, min, max2) {
  if (value === void 0) {
    return true;
  }
    ...
    InputBase,
    {
      ...
      onBlur: (event) => {
        ...
        if (clampBehavior === "blur" && typeof _value === "number") {
          const clampedValue = clamp(_value, min, max2);
          if (clampedValue !== _value) {
            setValue(clamp(_value, min, max2));
          }
        }
      ...
      isAllowed: (val) => {
        if (clampBehavior === "strict") {
          if (isAllowed) {
            return isAllowed(val) && isInRange(val.floatValue, min, max2);
          }
          return isInRange(val.floatValue, min, max2);
        }
...
var RingProgress = factory((_props, ref) => {
  ...
  const curves = getCurves({
    ...
    rootColor
  }).map(({ data, sum, root, lineRoundCaps, offset: offset3 }, index5) => (0, import_react372.createElement)(
    Curve,
    {
      ...data,
      key: index5,
        ...
        children: [
          (0, import_jsx_runtime282.jsx)(Combobox.Target, { targetType: searchable ? "input" : "button", autoComplete, children: (0, import_jsx_runtime282.jsx)(
            InputBase,
            {
              id: _id,
              ref,
              rightSection: rightSection || clearButton || (0, import_jsx_runtime282.jsx)(Combobox.Chevron, { size: size3, error: error2, unstyled }),
              rightSectionPointerEvents: rightSectionPointerEvents || (clearButton ? "all" : "none"),
              ...others,
              size: size3,
              __staticSelector: "Select",
...
function getPosition({ value, min, max: max2 }) {
  const position = (value - min) / (max2 - min) * 100;
...
// node_modules/@mantine/core/esm/components/Slider/Marks/Marks.mjs
function Marks({ marks, min, max: max2, disabled, value, offset: offset3, inverted }) {
  const { getStyles: getStyles2 } = useSliderContext();
  if (!marks) {
    return null;
  }
  const items = marks.map((mark, index5) => (0, import_react382.createElement)(
    Box,
    {
      ...
      __vars: { "--mark-offset": `${getPosition({ value: mark.value, min, max: max2 })}%` },
      key: index5
    },
...
var Slider = factory((_props, ref) => {
  ...
  const [_value, setValue] = useUncontrolled({
    value: typeof value === "number" ? clamp(value, min, max2) : value,
    defaultValue: typeof defaultValue === "number" ? clamp(defaultValue, min, max2) : defaultValue,
    finalValue: clamp(0, min, max2),
    onChange
  });
  const valueRef = (0, import_react384.useRef)(_value);
  const root = (0, import_react384.useRef)();
  const thumb = (0, import_react384.useRef)();
  const position = getPosition({ value: _value, min, max: max2 });
  ...
  const handleChange = (0, import_react384.useCallback)(
    ...
    [disabled, min, max2, step, precision, setValue]
  );
      ...
      switch (event.key) {
        case "ArrowUp": {
          ...
          const nextValue = getFloatingValue(
            Math.min(Math.max(_value + step, min), max2),
            precision
          );
          setValue(nextValue);
        ...
        case "ArrowRight": {
          ...
          const nextValue = getFloatingValue(
            Math.min(Math.max(dir === "rtl" ? _value - step : _value + step, min), max2),
            precision
          );
          setValue(nextValue);
          onChangeEnd == null ? void 0 : onChangeEnd(nextValue);
...
var RangeSlider = factory((_props, ref) => {
  ...
  const positions = [
    getPosition({ value: _value[0], min, max: max2 }),
    getPosition({ value: _value[1], min, max: max2 })
  ];
  const precision = _precision ?? getPrecision(step);
  const _setValue = (val) => {
    setValue(val);
    ...
    if (!disabled) {
      switch (event.key) {
        case "ArrowUp": {
          ...
          setRangedValue(
            getFloatingValue(
              Math.min(Math.max(valueRef.current[focusedIndex] + step, min), max2),
              precision
            ),
            focusedIndex,
            true
